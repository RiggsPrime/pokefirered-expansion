...
static const u8 sText_FORMS_Buttons_Submenu_PE[] = _("{DPAD_ANY}FORMs {A_BUTTON}CHECK {START_BUTTON}EVOs");
...
static const u32 sPokedexPlusHGSS_ScreenSearchKanto_Tilemap[] = INCBIN_U32("graphics/pokedex/hgss/tilemap_search_screen_hoenn.bin.smolTM");
...
static const struct SearchOptionText sDexSearchColorOptions[] =
{
    ...
    [BODY_COLOR_PINK + 1]   = {gText_DexEmptyString, gText_Pink},
    {},
};
...
#define temp_dexNum     vars[2]
    ...
    case ORDER_NUMERICAL:
        if (temp_isKantoDex)
        {
            ...
            {
                temp_dexNum = KantoToNationalDexNum(i + 1);
    ...
    case ORDER_ALPHABETICAL:
        for (i = 0; i < NATIONAL_DEX_COUNT; i++)
        {
            ...
            if ((!temp_isKantoDex || NationalToKantoDexNum(temp_dexNum) != 0) && GetSetPokedexFlag(temp_dexNum, FLAG_GET_SEEN))
            {
    ...
    case ORDER_HEAVIEST:
        for (i = NATIONAL_DEX_COUNT - 1; i >= 0; i--)
        {
            ...
            if ((!temp_isKantoDex || NationalToKantoDexNum(temp_dexNum) != 0) && GetSetPokedexFlag(temp_dexNum, FLAG_GET_CAUGHT))
            {
                sPokedexView->pokedexList[sPokedexView->pokemonListCount].dexNum = temp_dexNum;
                sPokedexView->pokedexList[sPokedexView->pokemonListCount].seen = TRUE;
...
static void CreateInterfaceSprites(u8 page)
{
    ...
    {
        // Kanto text
        CreateSprite(&sKantoNationalTextSpriteTemplate, LIST_RIGHT_SIDE_TEXT_X, 40 - LIST_RIGHT_SIDE_TEXT_Y_OFFSET - 6, 1);
        // Kanto seen
    ...
    else if (page == PAGE_MAIN)
    {
        ...
        // National text
        spriteId = CreateSprite(&sKantoNationalTextSpriteTemplate, LIST_RIGHT_SIDE_TEXT_X, 73 - LIST_RIGHT_SIDE_TEXT_Y_OFFSET - 6, 1);
        StartSpriteAnim(&gSprites[spriteId], 1);
        // National seen
...
static void PrintMonInfo(u32 num, u32 value, u32 owned, u32 newEntry)
{
    ...
    if (value == 0)
        value = NationalToKantoDexNum(num);
    else
        value = num;
...
static void Task_LoadStatsScreen(u8 taskId)
{
    ...
    case 6:
        ...
        PrintStatsScreen_NameGender(taskId, sPokedexListItem->dexNum, sPokedexView->dexMode == DEX_MODE_KANTO ? FALSE : TRUE);
...
static void PrintStatsScreen_Moves_Top(u8 taskId)
{
    ...
    {
        PrintStatsScreenTextSmall(WIN_STATS_MOVES_TOP, gText_ThreeHyphens, moves_x + 113, moves_y + 9);
        item = ITEM_LUCKY_EGG;
    }
...
static void PrintStatsScreen_Moves_Bottom(u8 taskId)
{
    ...
    if (gTasks[taskId].data[5] == 0)
    {
        ...
        if (power < 2)
            StringCopy(gStringVar1, gText_ThreeHyphens);
        else
    ...
    else //Appeal + Jam
    {
        DestroyCategoryIcon();
        //Appeal
        contest_effectValue = 0; // gContestEffects[GetMoveContestEffect(move)].appeal;
        ...
        //Jam
        contest_effectValue = 0; // gContestEffects[GetMoveContestEffect(move)].jam;
        if (contest_effectValue != 0xFF)
...
static void PrintStatsScreen_NameGender(u8 taskId, u32 num, u32 value)
{
    ...
    case 254:
        ...
        break;
    default:
        PrintStatsScreenTextSmall(WIN_STATS_NAME_GENDER, gText_ThreeHyphens, gender_x, gender_y);
        break;
    }
}
...
static void PrintStatsScreen_Left(u8 taskId)
{
    ...
    else
    {
        ...
        if (sPokedexView->sPokemonStats.eggGroup1 == EGG_GROUP_NO_EGGS_DISCOVERED || sPokedexView->sPokemonStats.eggGroup2 == EGG_GROUP_NO_EGGS_DISCOVERED) //Species without eggs (legendaries etc)
        {
            ...
            PrintStatsScreenTextSmall(WIN_STATS_LEFT, gText_ThreeHyphens, 78, base_y + base_y_offset*base_i);
        }
        else
        {
            PrintStatsScreenTextSmall(WIN_STATS_LEFT, sText_Stats_EggCycles, base_x, base_y + base_y_offset*base_i);
...
static void Task_LoadSearchMenu(u8 taskId)
{
    ...
    case 0:
        if (!gPaletteFade.active)
        {
            ...
            if (!IsNationalPokedexEnabled())
                CopyToBgTilemapBuffer(3, sPokedexPlusHGSS_ScreenSearchKanto_Tilemap, 0, 0);
            else
